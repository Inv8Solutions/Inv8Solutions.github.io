...
// 3. addBlogPost
content = content.replace(
  /const \{ data, error \} = await supabase\.from\('blogposts'\)\.insert\(\{([\s\S]*?)date: blogForm\.value\.date, read_time: blogForm\.value\.readTime,\n\s*\}\)\.select\('id'\)\.single\(\)\n\s*if \(error\) throw error\n\s*blogPosts\.value\.unshift\(\{ id: data\!\.id, \.\.\.blogForm\.value \}\)/g,
...
content = content.replace(
  /const \{ data: uploaded, error: upErr \} = await supabase\.storage\.from\('community-gallery'\)\.upload\(path, file, \{ contentType: file\.type \|\| 'image\/jpeg' \}\)\n\s*if \(upErr\) \{ console\.error\(upErr\); done\+\+; continue \}\n\s*const \{ data: \{ publicUrl \} \} = supabase\.storage\.from\('community-gallery'\)\.getPublicUrl\(uploaded\.path\)\n\s*await supabase\.from\('community_gallery'\)\.insert\(\{ url: publicUrl, storage_path: path, caption: '', order: base \+ done \}\)/g,
...
// 12. fetchInquiries
content = content.replace(
  /const \{ data \} = await supabase\.from\('inquiries'\)\.select\('\*'\)\.order\('created_at', \{ ascending: false \}\)\n\s*inquiries\.value = \(data \?\? \[\]\)\.map\(d => \(\{/g,
...
content = content.replace(
  /await supabase\.from\('inquiries'\)\.update\(\{ status: newStatus, updated_at: new Date\(\)\.toISOString\(\) \}\)\.eq\('id', selectedInquiry\.value\.id\)/g,
  "await updateDoc(doc(db, 'inquiries', selectedInquiry.value.id), { status: newStatus, updated_at: new Date().toISOString() })"
);

// 15. updateCallStatus
content = content.replace(
  /await supabase\.from\('calls'\)\.update\(\{ status, updated_at: new Date\(\)\.toISOString\(\) \}\)\.eq\('id', selectedCall\.value\.id\)/g,
  "await updateDoc(doc(db, 'calls', selectedCall.value.id), { status, updated_at: new Date().toISOString() })"
);

// 16. addProject
content = content.replace(
  /const \{ data: inserted, error: insErr \} = await supabase\.from\('sampleworks'\)\.insert\(\{([\s\S]*?)date: new Date\(\)\.toISOString\(\)\.split\('T'\)\[0\],\n\s*\}\)\.select\('id'\)\.single\(\)\n\s*if \(insErr\) throw insErr\n\s*projects\.value\.unshift\(\{ id: inserted\!\.id,/g,
...
// 17. deleteProject storage remove
content = content.replace(
  /if \(toRemove\.length\) await supabase\.storage\.from\('sampleworks'\)\.remove\(toRemove\)/g,
  "if (toRemove.length) await Promise.all(toRemove.map(path => deleteObject(storageRef(storage, path)).catch(() => {})))"
);
// 17.1 deleteProject db remove
content = content.replace(
  /await supabase\.from\('sampleworks'\)\.delete\(\)\.eq\('id', project\.id\)/g,
  "await deleteDoc(doc(db, 'sampleworks', project.id))"
);

// 18. updateProject fetching Single
content = content.replace(
  /const \{ data: d \} = await supabase\.from\('sampleworks'\)\.select\('\*'\)\.eq\('id', project\.id\)\.single\(\)/g,
...
content = content.replace(
  /await supabase\.from\('sampleworks'\)\.update\(update\)\.eq\('id', editingProjectId\.value\)/g,
  "await updateDoc(doc(db, 'sampleworks', editingProjectId.value), update)"
);

fs.writeFileSync('src/AdminPanel.vue', content);
console.log('Regex Conversion complete!');
